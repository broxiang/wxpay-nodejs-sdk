...
import { CombineMiniProgramService } from '../../src/services/combine-miniprogram.js';
...
describe('CombineMiniProgramService', () => {
  let service: CombineMiniProgramService;
  let mockClient: {
  ...
  beforeEach(() => {
    ...
    service = new CombineMiniProgramService(mockClient as unknown as WxPayClient);
  });

  describe('createOrder', () => {
    it('should call POST /v3/combine-transactions/jsapi with correct parameters', async () => {
      const request: CreateMiniProgramCombineOrderRequest = {
        combine_appid: 'wxd678efh567hg6787',
        combine_mchid: '1230000109',
        combine_out_trade_no: 'MINIPROGRAM_COMBINE_20240609_001',
    ...
    it('should handle request without combine_payer_info (小程序隐式提供用户身份)', async () => {
      const request: CreateMiniProgramCombineOrderRequest = {
        combine_appid: 'wx1234567890abcdef',
        combine_mchid: '1230000109',
        combine_out_trade_no: 'MINI_NO_PAYER_001',
      ...
      mockClient.post.mockResolvedValue({
        status: 200,
        headers: {},
        data: { prepay_id: 'wx_min_prepay_002' },
      });

      const result = await service.createOrder(request);

      expect(mockClient.post).toHaveBeenCalledWith('/v3/combine-transactions/jsapi', request);
      expect(result.data.prepay_id).toBe('wx_min_prepay_002');
    });

    it('should handle request with profit_sharing enabled on sub-orders', async () => {
      const request: CreateMiniProgramCombineOrderRequest = {
        combine_appid: 'wxd678efh567hg6787',
        combine_mchid: '1230000109',
        combine_out_trade_no: 'MINI_PROFIT_SHARING_001',
        notify_url: 'https://example.com/notify',
        sub_orders: [
          {
            mchid: '1230000109',
            out_trade_no: 'SUB_PS_001',
            description: '需要分账的商品',
            amount: { total_amount: 500, currency: 'CNY' },
            attach: '分账订单',
            settle_info: { profit_sharing: true },
          },
          {
            mchid: '1230000110',
            out_trade_no: 'SUB_PS_002',
            description: '需要分账的商品2',
            amount: { total_amount: 300, currency: 'CNY' },
            attach: '分账订单2',
            settle_info: { profit_sharing: true },
          },
        ],
      ...
      mockClient.post.mockResolvedValue({
        status: 200,
        headers: {},
        data: { prepay_id: 'wx_ps_prepay_001' },
      });
  ...
  describe('queryOrderById', () => {
    it('should call GET /v3/combine-transactions/out-trade-no/{combineOutTradeNo}', async () => {
      const params = {
        combineOutTradeNo: 'MINIPROGRAM_COMBINE_20240609_001',
      };
      ...
      expect(mockClient.get).toHaveBeenCalledWith(
        '/v3/combine-transactions/out-trade-no/MINIPROGRAM_COMBINE_20240609_001',
      );
      expect(result.data.combine_out_trade_no).toBe('MINIPROGRAM_COMBINE_20240609_001');
      expect(result.data.sub_orders).toHaveLength(2);
      expect(result.data.sub_orders?.[0]?.trade_state).toBe('SUCCESS');
      expect(result.data.sub_orders?.[0]?.trade_type).toBe('JSAPI');
    });

    it('should return NOTPAY state for unpaid orders', async () => {
      const params = {
        combineOutTradeNo: 'MINI_UNPAID_ORDER',
      };
      ...
      mockClient.get.mockResolvedValue({
        ...
        data: {
          combine_appid: 'wxd678efh567hg6787',
          combine_mchid: '1230000109',
          combine_out_trade_no: 'MINI_UNPAID_ORDER',
          sub_orders: [
            {
              mchid: '1230000109',
              out_trade_no: 'SUB_ORDER_001',
    ...
    it('should return CLOSED state for closed orders', async () => {
      const params = {
        combineOutTradeNo: 'MINI_CLOSED_ORDER',
      };
      ...
      mockClient.get.mockResolvedValue({
        ...
        data: {
          combine_appid: 'wxd678efh567hg6787',
          combine_mchid: '1230000109',
          combine_out_trade_no: 'MINI_CLOSED_ORDER',
          sub_orders: [
            {
              mchid: '1230000109',
              out_trade_no: 'SUB_ORDER_001',
    ...
    it('should return order with promotion_detail', async () => {
      const params = {
        combineOutTradeNo: 'MINI_PROMOTION_ORDER',
      };
      ...
      mockClient.get.mockResolvedValue({
        ...
        data: {
          combine_appid: 'wxd678efh567hg6787',
          combine_mchid: '1230000109',
          combine_out_trade_no: 'MINI_PROMOTION_ORDER',
          sub_orders: [
            {
              mchid: '1230000109',
              out_trade_no: 'SUB_ORDER_001',
  ...
  describe('closeOrder', () => {
    it('should call POST /v3/combine-transactions/out-trade-no/{combineOutTradeNo}/close', async () => {
      ...
      expect(mockClient.post).toHaveBeenCalledWith(
        '/v3/combine-transactions/out-trade-no/MINIPROGRAM_COMBINE_20240609_001/close',
        request,
      );
  ...
  describe('queryRefund', () => {
    ...
    it('should return ABNORMAL status for abnormal refunds', async () => {
      const params = {
        outRefundNo: 'REFUND_ABNORMAL',
      };
      ...
      mockClient.get.mockResolvedValue({
        ...
        headers: {},
        data: {
          refund_id: '5030000118202406090001234571',
          out_refund_no: 'REFUND_ABNORMAL',
          transaction_id: '4200001234567890',
          out_trade_no: 'SUB_ORDER_001',
          channel: 'ORIGINAL' as const,
          user_received_account: '',
          create_time: '2024-06-09T12:50:00+08:00',
      ...
      expect(result.data.status).toBe('ABNORMAL');
    });
  });

  describe('applyAbnormalRefund', () => {
    it('should call POST /v3/refund/domestic/refunds/{refundId}/apply-abnormal-refund with USER_BANK_CARD type', async () => {
      const refundId = '5030000118202406090001234571';
      ...
      const expectedResponse = {
        ...
        data: {
          ...
          transaction_id: '4200001234567890',
          out_trade_no: 'SUB_ORDER_001',
          channel: 'OTHER_BANKCARD' as const,
          user_received_account: '张三招商银行',
      ...
      expect(result.data.status).toBe('PROCESSING');
      expect(result.data.channel).toBe('OTHER_BANKCARD');
    });

    it('should call POST with MERCHANT_BANK_CARD type', async () => {
      const refundId = '5030000118202406090001234572';
      const request: ApplyAbnormalRefundRequest = {
        out_refund_no: 'REFUND_ABNORMAL_2',
      ...
      mockClient.post.mockResolvedValue({
        ...
        headers: {},
        data: {
          refund_id: '5030000118202406090001234572',
          out_refund_no: 'REFUND_ABNORMAL_2',
          transaction_id: '4200001234567890',
          out_trade_no: 'SUB_ORDER_001',
          channel: 'OTHER_BALANCE' as const,
          user_received_account: '',
          create_time: '2024-06-09T13:05:00+08:00',
          status: 'PROCESSING' as const,
          funds_account: 'AVAILABLE' as const,
          amount: {
            total: 10,
  ...
  describe('tradeBill', () => {
    ...
    it('should handle REFUND type bill', async () => {
      const params = {
        bill_date: '2024-06-09',
        bill_type: 'REFUND' as const,
      };
      ...
      expect(mockClient.get).toHaveBeenCalledWith('/v3/bill/tradebill', params);
      expect(result.data.hash_value).toBe('def456');
    });
  });
  ...
  describe('fundFlowBill', () => {
    ...
    it('should handle OPERATION account_type', async () => {
      const params = {
        bill_date: '2024-06-09',
      ...
      mockClient.get.mockResolvedValue({
        ...
        data: {
          hash_type: 'SHA1',
          hash_value: 'oper123',
          download_url: 'https://api.mch.weixin.qq.com/v3/billdownload/file?token=oper',
        },
      });

      await service.fundFlowBill(params);
  ...
  describe('downloadBill', () => {
    ...
    it('should handle GZIP compressed bill data', async () => {
      const downloadUrl = 'https://api.mch.weixin.qq.com/v3/billdownload/file?token=gzip';
      const gzipData = Buffer.from([0x1f, 0x8b, 0x08]); // GZIP magic bytes
